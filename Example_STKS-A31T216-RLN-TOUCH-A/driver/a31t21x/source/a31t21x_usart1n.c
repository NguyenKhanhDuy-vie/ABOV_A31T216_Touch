...
* @brief        A31T21x USART1n Driver Code
*
    ...
    uint32_t fd;
		...
		if ((cfg->mode == USART_MODE_ASYNC) || (cfg->mode == USART_MODE_SYNC))
		{
				...
				if (cfg->mode == USART_MODE_SYNC)
				{
						...
						{
								case USART_SYNC_MODE_CPOL_1_CPHA_0:
										usart->CR1 |= (1 << 7);
										break;
								case USART_SYNC_MODE_CPOL_0_CPHA_0:
										usart->CR1 &= ~(1 << 7);
										break;
								default:
										DRIVER_ASSERT(0);
		...
		else if (cfg->mode == USART_MODE_SPI)
		{
				...
				switch (cfg->sync_mode)
				{
						case USART_SYNC_MODE_CPOL_1_CPHA_0:
								...
								cfg_val &= ~(1 << 6);
								break;
						case USART_SYNC_MODE_CPOL_1_CPHA_1:
								cfg_val |= (1 << 7);
								cfg_val |= (1 << 6);
								break;
						case USART_SYNC_MODE_CPOL_0_CPHA_0:
								...
								cfg_val &= ~(1 << 6);
								break;
						case USART_SYNC_MODE_CPOL_0_CPHA_1:
								cfg_val &= ~(1 << 7);
								cfg_val |= (1 << 6);
								break;
						default:
